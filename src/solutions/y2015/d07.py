...
def p1(input_string: str) -> str:
    (wires, gates) = setup_wires_and_gates(input_string)

    return process_wires(wires, gates)
...
def setup_wires_and_gates(input_string):
    # letter : value
    wires = {}
    # output : (s1, s2, op)
    gates = {}
...
def process_wires(wires, gates):
    gates_to_process = list(gates)

    def have_gates_to_process()->bool:
        return len(gates_to_process) > 0

    while have_gates_to_process():

        dest = gates_to_process.pop(0)
